"""Step 06: Stamp UML stereotypes derivable from the XSD.

Guaranteed (structural) rules:
- complexType whose extension chain reaches gml:AbstractFeatureType
  -> <<FeatureType>>
- chain reaches gml:AbstractGMLType but not AbstractFeatureType
  -> <<ObjectType>>
- complexType with no GML ancestor -> <<DataType>> (default convention)
- simpleType restricting a string with enumeration facets -> <<Enumeration>>
- simpleType with <union> -> <<Union>>
- any other simpleType (restricted built-in) -> <<BasicType>> (heuristic)

Not derivable and therefore skipped: <<TopLevelFeatureType>> (a
designation, no structural trace) and <<CodeList>> (external OGC
dictionaries, absent from the schema).

Simple types are added as new stereotyped class boxes. Property wrappers
and ADE hooks are encoding artifacts and get no stereotype.

Usage:
    python3 step06_stereotypes.py [xsd_path] [output_path]
"""
...
from step02_inheritance import build_graph, find_schema_by_namespace, parse_inheritance
...
def class_stereotypes(classes, graph, prefix) -> dict:
    """Stereotype for each complexType class, from its GML ancestry."""
    child_to_parent = graph
    result = {}
    for cls in classes:
        qname = qualify(cls["name"], prefix)
        cur, seen, gml_ancestors = qname, set(), set()
        while cur in child_to_parent and cur not in seen:
            seen.add(cur)
            cur = child_to_parent[cur]
            if cur.startswith("gml:"):
                gml_ancestors.add(cur)
        if "gml:AbstractFeature" in gml_ancestors:
            result[qname] = "FeatureType"
        elif gml_ancestors:
            result[qname] = "ObjectType"
        else:
            result[qname] = "DataType"
    return result


def parse_simple_types(xsd_path: Path) -> list[dict]:
    """Top-level simpleTypes with their stereotype and enumeration values."""
    ...
    for st in root.findall(f"{XSD_NS}simpleType"):
        ...
        if not name:
            continue
        values = [e.get("value") for e in st.findall(f".//{XSD_NS}enumeration")]
        if st.find(f"{XSD_NS}union") is not None:
            stereotype = "Union"
        elif values:
            stereotype = "Enumeration"
        else:
            stereotype = "BasicType"
        result.append({"name": strip_type_suffix(name), "stereotype": stereotype,
                       "values": values})
    return result


def to_plantuml(classes, inheritance, attributes, relations, promoted,
                stereotypes, simple_types, title, prefix, subtitle=None):
    ...
    if simple_types:
        ...
        for st in simple_types:
            lines.append(f'class "{qualify(st["name"], prefix)}" <<{st["stereotype"]}>>')
    if prefix:
        lines.append("}")
...
def main() -> None:
    ...
    out_path.write_text(
        to_plantuml(classes, inheritance, attributes, relations, promoted,
                    stereotypes, simple_types, diagram_title(xsd_path), prefix,
                    target_namespace(xsd_path)),
        encoding="utf-8")
    counts = {}
    stamped = [st for c in classes
               if not (c["name"].startswith("ADEOf") or c["name"].endswith("Property"))
               for st in [stereotypes.get(qualify(c["name"], prefix))] if st]
    for st in stamped + [s["stereotype"] for s in simple_types]:
        counts[st] = counts.get(st, 0) + 1
    summary = ", ".join(f"{n} <<{k}>>" for k, n in sorted(counts.items()))
    print(f"stereotypes assigned: {summary}; written to {out_path}")
